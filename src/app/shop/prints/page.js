...
export default function PrintsPage() {
  ...
  const { visible, setVisible, rememberCard } = useGridRestore(`shop-prints:${tag}:${query}`, PAGE_SIZE, !loading);
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="mt-6 text-center">
        <h1 className="text-2xl sm:text-3xl text-[var(--color-accent)]" style={{ fontFamily: "var(--font-fraunces)" }}>Prints</h1>
        <p className="mt-4 text-gray-600 max-w-xl mx-auto">
          Archival-quality prints in four sizes. Choose Matte or Glossy finish.
        </p>
      </div>
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {filtered.slice(0, visible).map((item) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-500">Print · 4 sizes available</p>
                    <p className="mt-1 text-base font-semibold text-gray-900">
                      From €{item.prints?.sizes?.[0]?.price ?? 12}
                    </p>
                  </ArtworkCard>
                ))}
              </div>
