...
export default function OilPaintingsPage() {
  ...
  const { visible, setVisible, rememberCard } = useGridRestore(`portfolio-oil:${tag}:${query}`, PAGE_SIZE, !loading);
  ...
  useEffect(() => {
    getAllArtworks().then((all) => {
      setPaintings(all.filter((a) => a.category === "oil"));
      setLoading(false);
    });
  }, []);
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="mt-6 text-center">
        <h1 className="text-2xl sm:text-3xl text-[var(--color-accent)]" style={{ fontFamily: "var(--font-fraunces)" }}>Oil Paintings</h1>
        <p className="mt-4 text-gray-600 max-w-xl mx-auto">Original oil paintings on canvas and board.</p>
      </div>
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {filtered.slice(0, visible).map((artwork) => (
                  <ArtworkCard
                    ...
                    artwork={artwork}
                    type="original"
                    href={`/portfolio/oil-paintings/${artwork.id}`}
                  ...
                  >
                    ...
                    <div className="mt-1">
                      {artwork.original?.available ? (
                        <p className="text-sm font-medium text-gray-900">€{artwork.original.price}</p>
                      ) : (
                        <p className="text-sm">
                          <span className="font-medium text-gray-400">€{artwork.original?.price}</span>
                          <span className="ml-2 text-[var(--color-accent)]">Sold</span>
                        </p>
                      )}
                    </div>
                  </ArtworkCard>
